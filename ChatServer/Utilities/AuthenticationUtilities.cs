...
namespace ChatServer.Utilities
{
    public class AuthenticationUtilities
    {
        ...
        public static ClientUserDto Authenticate(UserLoginDto user)
        {
            ...
            var existingUser = UserMocks.users.FirstOrDefault(u => u.Username == user.Username && u.Password == user.Password);
        ...
        public static string GenerateToken(ClientUserDto user, IConfiguration configuration)
        {
            if(user == null)
            {
                return null;
            ...
            var token = new JwtSecurityToken(configuration["Jwt:Issues"],
